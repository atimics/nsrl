...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let suffix_memory_present = suffix_memory_is_installed(&model);
...
fn apply_ablation(
    ...
    match ablation {
        ...
        EvalAblation::TransformerOnly => {
            // The calibrated suffix memory stores both its header and corpus
            // payload in the position tensor. A promotion-grade transformer-
            // only artifact must remove the payload physically, not merely
            // make it unreachable by clearing the magic header.
...
fn annotate_trace(
    ...
    let metadata = format!(
        "\"ablation\":{{\"mode\":\"{}\",\"source_model_hash\":\"0x{:016x}\",\"evaluated_model_hash\":\"0x{:016x}\",\"suffix_memory_present\":{}}},{}",
        ablation.as_str(),
        source_model_hash,
        evaluated_model_hash,
        suffix_memory_present,
