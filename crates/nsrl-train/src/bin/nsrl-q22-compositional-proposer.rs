...
const MODEL_MAGIC: &[u8; 8] = b"NSRLQ22C";
const MODEL_VERSION: u32 = 1;
const MODEL_SCHEMA: &str = "nsrl.q22_compositional_integer_class_head.v1";
const TRAIN_TRACE_SCHEMA: &str = "nsrl.q22_compositional_integer_class_head_train.v1";
const PREDICT_TRACE_SCHEMA: &str = "nsrl.q22_compositional_integer_class_head_predict.v1";
...
fn main() {
    if let Err(error) = run() {
        eprintln!("nsrl-q22-compositional-proposer: {error}");
        std::process::exit(1);
    }
}
...
fn load_blind_inputs(path: &Path, expected_records: usize) -> Result<Vec<BlindInput>, String> {
    ...
    for (offset, line) in lines.enumerate() {
        ...
        if input.contains('\t')
            || input.is_empty()
            || !id.starts_with("quantity-composition/promotion/")
...
fn usage() {
    println!(
        "Usage:\n  nsrl-q22-compositional-proposer train --manifest PATH --encoded PATH --seed N [--epochs N] --model-out PATH [--trace-out PATH]\n  nsrl-q22-compositional-proposer predict --manifest PATH --model PATH --inputs BLINDED.tsv --predictions-out PATH [--trace-out PATH]\n  nsrl-q22-compositional-proposer inspect --model PATH\n\nTrain reads only the frozen Solomon Q22 compositional encoding. Predict accepts only the two-column compositional blinded ID/input surface."
    );
}
...
mod tests {
    ...
    fn integer_head_learns_operation_routing_and_round_trips() {
        let records = vec![
            record(
                "Route this quantity case. Background: equal groups. Actual case: Combine 2 with 3. Respond with the operation code.",
                "quantity.add",
            ),
            record(
                "Route this quantity case. Background: convert units. Actual case: Increase -5 by 8. Respond with the operation code.",
                "quantity.add",
            ),
            record(
                "Route this quantity case. Background: join amounts. Actual case: Scale 2 by 3. Respond with the operation code.",
                "quantity.multiply",
            ),
            record(
                "Route this quantity case. Background: fractional shares. Actual case: Use -5 groups of 8. Respond with the operation code.",
                "quantity.multiply",
            ),
            record(
                "Route this quantity case. Background: isolate x. Actual case: Combine shares 1/2 and 1/3. Respond with the operation code.",
                "quantity.add-rational",
            ),
            record(
                "Route this quantity case. Background: scale values. Actual case: Pool -2/5 with 7/9. Respond with the operation code.",
                "quantity.add-rational",
            ),
            record(
                "Route this quantity case. Background: add values. Actual case: Express 2 kilograms in grams. Respond with the operation code.",
                "quantity.convert",
            ),
            record(
                "Route this quantity case. Background: equal groups. Actual case: Restate 3 meters in centimeters. Respond with the operation code.",
                "quantity.convert",
            ),
            record(
                "Route this quantity case. Background: convert units. Actual case: Find x in 2*x+1=5. Respond with the operation code.",
                "quantity.solve-linear",
            ),
            record(
                "Route this quantity case. Background: join shares. Actual case: Isolate x in 7*x+-2=33. Respond with the operation code.",
                "quantity.solve-linear",
            ),
        ];
        ...
        let (model, _) = train_model(&records, &manifest, 3, 40).expect("train");
        for row in &records {
            assert_eq!(
    ...
    fn features_and_shuffle_are_deterministic_and_seeded() {
        assert_eq!(
            features("Route this quantity case. Actual case: Combine 2 with 3."),
            features(" Route this quantity case. Actual case: Combine 2 with 3. ")
        );
