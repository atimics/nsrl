...
pub const Q22_MANIFEST_SCHEMA: &str = "nsrl.q22_compositional_shared_task_manifest.v1";
pub const Q22_CONTRACT_ID: &str = "zero-solomon.q22-compositional-routing.v1";
pub const Q22_ENCODING_ID: &str = "solomon.q22-compositional-operation.byte-v1";
pub const Q22_VERIFIER_ID: &str = "verifier://solomon/q22-compositional-operation/v1";
...
const TRAINING_SCHEMA: &str = "zero.faculty_operation_request.v2";
const PREVIOUS_SUMMARY: &str = "quantity routing has no prior committed result";
...
const COMMON_PREFIX: &str = "Route this quantity case. ";
...
pub struct Q22Check {
    ...
    pub per_class_cases: [usize; OPERATIONS.len()],
    pub per_class_exact: [usize; OPERATIONS.len()],
}
...
impl Q22Check {
    ...
    pub fn to_json_line(&self) -> String {
        let per_class = OPERATIONS
            .iter()
            .enumerate()
            .map(|(index, operation)| {
                let rate = self.per_class_exact[index].saturating_mul(1_000_000)
                    / self.per_class_cases[index].max(1);
                format!("\"{operation}\":{rate}")
            })
            .collect::<Vec<_>>()
            .join(",");
        format!(
            "{{\"schema\":\"nsrl.q22_compositional_shared_task_check.v1\",\"contract\":\"{Q22_CONTRACT_ID}\",\"eval_sha256\":\"{}\",\"cases\":{},\"operation_exact\":{},\"operation_exact_rate_ppm\":{},\"per_class_exact_rate_ppm\":{{{per_class}}},\"valid\":true}}\n",
            self.eval_sha256,
            self.cases,
...
pub fn encode_q22_dataset(path: &Path, manifest: &Q22Manifest) -> Result<Vec<u8>, String> {
    ...
    for record in records {
        encoded.extend_from_slice(b"[q22-compositional]\ninput: ");
...
pub fn load_q22_encoded_training_records(
    ...
    let text =
        std::str::from_utf8(&encoded).map_err(|_| "Q22 compositional encoding must be UTF-8")?;
    if !text.ends_with('\n') {
        return Err("Q22 compositional encoding must end with one newline".to_string());
    }
    let lines = text.lines().collect::<Vec<_>>();
    if !lines.len().is_multiple_of(4) {
        return Err("Q22 compositional encoding must contain four lines per record".to_string());
    }
    ...
    for (index, chunk) in lines.chunks_exact(4).enumerate() {
        ...
        if chunk[0] != "[q22-compositional]"
            || chunk[3] != "[/q22-compositional]"
            || input.is_empty()
            || input.contains(['\t', '\r', '\n'])
    ...
    if records.len() != manifest.train_records {
        return Err(format!(
            "Q22 compositional encoding has {} records, expected {}",
            records.len(),
...
pub fn blind_q22_evaluation(path: &Path, manifest: &Q22Manifest) -> Result<Vec<u8>, String> {
    ...
    for (offset, line) in lines.enumerate() {
        ...
        if fields.len() != 8
            ...
            || !fields[0].starts_with("quantity-composition/promotion/")
            || fields[0].contains(['\r', '\n'])
            || fields[3].contains(['\t', '\r', '\n'])
            || !valid_model_input(fields[3])
            || !ids.insert(fields[0])
        {
...
pub fn check_q22_predictions(
    ...
    let operation_exact = expected
        ...
        .count();
    let mut per_class_cases = [0_usize; OPERATIONS.len()];
    let mut per_class_exact = [0_usize; OPERATIONS.len()];
    for (id, operation) in &expected {
        let index = operation_index(operation).ok_or("unsupported frozen operation")?;
        per_class_cases[index] += 1;
        if predictions.get(id) == Some(operation) {
            per_class_exact[index] += 1;
        }
    }
    Ok(Q22Check {
...
fn parse_training_records(
    ...
    for (index, line) in text.lines().enumerate() {
        ...
        if string_field(&value, "schema", index + 1)? != TRAINING_SCHEMA
            ...
            || string_field(&value, "curriculum", index + 1)? != "quantity-compositional-routing-v1"
            || string_field(&value, "previous_summary", index + 1)? != PREVIOUS_SUMMARY
            || !id.starts_with("quantity-composition/train/")
            || !valid_model_input(&input)
        {
            return Err(format!(
        ...
        let request = string_field(&value, "request", index + 1)?;
    ...
    if expected_count % OPERATIONS.len() != 0 {
        return Err("Q22 training count must be divisible by its class count".to_string());
    }
    let per_class = expected_count / OPERATIONS.len();
    for operation in OPERATIONS {
        if records
            .iter()
            .filter(|record| record.model_request == operation)
            .count()
            != per_class
        {
            return Err(format!(
                "Q22 training class balance drifted for {operation}"
            ));
        }
    }
    Ok(records)
...
fn parse_evaluation(
    ...
    for (offset, line) in lines.enumerate() {
        ...
        let expected = expected_for_request(fields[5])?;
    ...
    if expected_count % OPERATIONS.len() != 0 {
        return Err("Q22 evaluation count must be divisible by its class count".to_string());
    }
    let per_class = expected_count / OPERATIONS.len();
    for operation in OPERATIONS {
        if rows
            .values()
            .filter(|value| value.as_str() == operation)
            .count()
            != per_class
        {
            return Err(format!(
                "Q22 evaluation class balance drifted for {operation}"
            ));
        }
    }
    Ok(rows)
...
fn expected_for_request(request: &str) -> Result<Expected, String> {
    let parts = request.split_whitespace().collect::<Vec<_>>();
    match parts.as_slice() {
        ["quantity.add", a, b] => {
            let a = integer(a, request)?;
            let b = integer(b, request)?;
            expected("add", format!("{a} {b}"), format!("result {}", a + b))
        }
        ["quantity.multiply", a, b] => {
            let a = integer(a, request)?;
            let b = integer(b, request)?;
            expected("multiply", format!("{a} {b}"), format!("result {}", a * b))
        }
        ["quantity.add-rational", left, right] => {
            let (a, b) = fraction(left, request)?;
            let (c, d) = fraction(right, request)?;
        ...
        [
            ...
            let value = positive_integer(value, request)?;
        ...
        ["quantity.solve-linear", coefficient, offset, result] => {
            expected_solve_request(coefficient, offset, result, request)
        }
        _ => Err(format!("invalid Q22 request {request}")),
    }
}

fn expected_solve_request(
    coefficient: &str,
    offset: &str,
    result: &str,
    request: &str,
) -> Result<Expected, String> {
    let coefficient = positive_integer(coefficient, request)?;
    let offset = integer(offset, request)?;
    let result = integer(result, request)?;
    let numerator = result - offset;
    if numerator % coefficient != 0 {
        return Err(format!("non-integral Q22 solution {request}"));
    }
    expected(
        "solve-linear",
...
fn valid_model_input(input: &str) -> bool {
    input.starts_with(COMMON_PREFIX)
        && input.ends_with(" Respond with the operation code.")
        && input.contains(" Background: ")
        && input.contains(" Actual case: ")
...
mod tests {
    ...
    fn kernel_covers_all_five_q22_operations() {
        let cases = [
            ("quantity.add -2 7", "quantity.add", "result 5"),
            ("quantity.multiply -3 9", "quantity.multiply", "result -27"),
            (
                "quantity.add-rational -5/19 -22/9",
                "quantity.add-rational",
                "result -463/171",
            ),
            (
                "quantity.convert 12 kg-to-g",
                "quantity.convert",
                "result 12000 g",
            ),
            (
                "quantity.solve-linear 7 -2 33",
                "quantity.solve-linear",
                "x 5",
            ),
        ];
        for (request, operation, artifact) in cases {
            let expected = expected_for_request(request).expect("valid task");
    ...
    fn blinding_removes_every_label_bearing_column() {
        let evaluation = format!(
            "{EVAL_HEADER}\nquantity-composition/promotion/000001\tquantity\t{PREVIOUS_SUMMARY}\tRoute this quantity case. Background: ignored. Actual case: Combine -2 with 7. Respond with the operation code.\tquantity.add\tquantity.add -2 7\tresult 5\tkernel committed result 5\n"
        );
        ...
        assert_eq!(
            blinded,
            b"id\tinput\nquantity-composition/promotion/000001\tRoute this quantity case. Background: ignored. Actual case: Combine -2 with 7. Respond with the operation code.\n"
        );
    ...
    fn encoded_loader_requires_the_frozen_solomon_surface() {
        let encoded = b"[q22-compositional]\ninput: Route this quantity case. Background: ignored. Actual case: Find x. Respond with the operation code.\noperation: quantity.solve-linear\n[/q22-compositional]\n";
        let path = std::env::temp_dir().join(format!(
        ...
        assert_eq!(
            records[0].input,
            "Route this quantity case. Background: ignored. Actual case: Find x. Respond with the operation code."
        );
