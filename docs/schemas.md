...
## `nsrl.bounty_automation_policy.v1`

Defined by `protocol/bounty-automation-policy-v1.schema.json`, reduced by
`scripts/lib/model-localnet-v1.mjs`, and exercised by
`scripts/check-bounty-automation-v1.mjs`.

Purpose: bind a sponsor, proposer, and keeper to one promoted model lineage and
an immutable metric improvement, per-cycle funding, lifetime spend cap,
approval threshold, concurrency/cycle/cooldown limits, and sealed-auction
terms. The keeper derives the successor recipe and cannot choose values outside
this signed envelope.
...
`web/launches/automation.json`, generated by
`scripts/build-bounty-automation-site.mjs`, adds the signed policy, keeper plan,
one-time reserved funding, deterministic successor recipe, four-event keeper
result, and 84-event public replay. It contains no private keys.
