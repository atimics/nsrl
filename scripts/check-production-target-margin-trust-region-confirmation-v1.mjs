...
assert(contract.schema === "nsrl.production_target_margin_trust_region_contract.v1"
  && development.schema
    === "nsrl.production_target_margin_trust_region_development_gate.v1"
...
"target-margin trust-region development gate is invalid");
...
"target-margin trust-region confirmation schema is invalid");
...
"target-margin trust-region confirmation binding mismatch");
...
"target-margin trust-region rollout geometry is invalid");
...
const result = {
  schema: "nsrl.production_target_margin_trust_region_confirmation_gate.v1",
...
if (config.check) {
  assert(fs.readFileSync(config.out, "utf8") === output,
    "target-margin trust-region confirmation gate does not byte-replay");
} else {
