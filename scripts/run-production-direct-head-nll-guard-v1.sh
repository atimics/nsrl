...
name="p10m-direct-head-nll-guard-v1"
...
echo "direct-head NLL guard: exact bounded run"
"$binary" direct-head-train \
  ...
  --direct-head-dev-guard --seed 0

echo "direct-head NLL guard: exact full rerun replay"
"$binary" direct-head-train \
  --tokenizer "$tokenizer" --tokens "$train_tokens" \
  --model "$source_model" \
...
echo "direct-head NLL guard: public development stop/go gate"
...
echo "direct-head NLL guard: public test confirmation gate"
"$binary" evaluate-canonical \
...
node scripts/check-production-direct-head-nll-guard-confirmation-v1.mjs \
...
echo "direct-head NLL guard: authorized open-generation checks"
for role in source candidate; do
  if [[ "$role" == "source" ]]; then
    model="$source_model"
  else
...
node scripts/freeze-production-direct-head-nll-guard-quality-v1.mjs \
