...
trace="${NSRL_PRODUCTION_GRADIENT_ALIGNMENT_TRACE:-benchmarks/production-model-v1/p10m-gradient-lane-alignment-v1.json}"

cargo build --release -p nsrl-train --bin nsrl-production-model
target/release/nsrl-production-model gradient-alignment-audit \
  --tokenizer "$tokenizer" --tokens "$tokens" --model "$model" --trace "$trace" \
  --context-tokens 64 --max-windows 2 --transfer-windows 2 \
  --coordinates-per-group 1 --seed 29 \
...
echo "production p10m gradient-lane alignment audit completed: $trace"
