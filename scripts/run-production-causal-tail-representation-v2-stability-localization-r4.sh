...
name="p10m-causal-tail-representation-v2-stability-localization-r4"
...
cleanup() {
  ...
  if ((status == 0)); then
    ...
    echo "single-step stability localization failed; retained work directory: $work_dir" >&2
  fi
}
trap cleanup EXIT
...
declare -a pids=()
for pair in "429 1" "430 2" "431 3"; do
  read -r total additional <<< "$pair"
  echo "single-step stability localization phase: train step $total"
...
for total in 429 430 431; do
  model="$work_dir/model-step-${total}.nsrlpm"
  echo "single-step stability localization phase: audit step $total"
