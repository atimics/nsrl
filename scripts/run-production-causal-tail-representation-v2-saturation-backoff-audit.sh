...
name="p10m-causal-tail-representation-v2-saturation-backoff-audit"
...
target/release/nsrl-production-saturation-backoff-audit \
  ...
  --output-backward-shifts 8,9,10,11,12,13,14,15,16 \
  --trace "$audit" >/dev/null

node scripts/freeze-production-saturation-backoff-audit-v1.mjs \
