...
name="p10m-causal-tail-representation-v7-signed-block"
...
cleanup() {
  ...
  if ((status == 0)); then
    ...
    echo "representation-v7 signed-block run failed; retained work directory: $work_dir" >&2
  fi
}
trap cleanup EXIT
...
echo "representation-v7 signed-block phase: train 2,048 windows"
...
echo "representation-v7 signed-block phase: development and health audit"
