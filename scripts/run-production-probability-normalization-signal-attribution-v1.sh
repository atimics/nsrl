...
out_dir="${NSRL_PRODUCTION_NORMALIZATION_ATTRIBUTION_OUT_DIR:-data/experiments/production-model-v1/p10m-probability-normalization-signal-attribution}"
checkpoint_out="${NSRL_PRODUCTION_NORMALIZATION_ATTRIBUTION_CHECKPOINT_OUT:-benchmarks/production-model-v1/p10m-probability-normalization-signal-attribution.json}"
...
if [[ ! -s "$out_dir/audit.json" ]]; then
  "$binary" probability-normalization-signal-attribution-audit \
...
node scripts/freeze-production-probability-normalization-signal-attribution-v1.mjs \
  --run-dir "$out_dir" --out "$checkpoint_out"
echo "production-model-v1 p10m normalization signal attribution completed"
