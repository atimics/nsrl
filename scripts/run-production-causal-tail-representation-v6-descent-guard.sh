...
name="p10m-causal-tail-representation-v6-descent-guard"
...
cleanup() {
  ...
  if ((status == 0)); then
    ...
    echo "representation-v6 descent-guard run failed; retained work directory: $work_dir" >&2
  fi
}
trap cleanup EXIT
...
echo "representation-v6 descent-guard phase: train 2,048 windows"
...
echo "representation-v6 descent-guard phase: development and health audit"
