...
"direct-head development gate is invalid");
...
"direct-head confirmation schema is invalid");
...
"direct-head confirmation binding mismatch");
...
"direct-head confirmation rollout geometry is invalid");
...
const result = {
  schema: "nsrl.production_direct_head_nll_guard_confirmation_gate.v1",
...
if (config.check) {
  assert(fs.readFileSync(config.out, "utf8") === output,
    "direct-head confirmation gate does not byte-replay");
} else {
