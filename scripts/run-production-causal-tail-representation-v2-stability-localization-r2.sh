...
name="p10m-causal-tail-representation-v2-stability-localization-r2"
...
source_model="data/experiments/production-model-v1/p10m-causal-tail-representation-v2-stability-localization/model-step-384.nsrlpm"
source_optimizer="data/experiments/production-model-v1/p10m-causal-tail-representation-v2-stability-localization/optimizer-step-384.nsrlpo"
...
cleanup() {
  ...
  if ((status == 0)); then
    ...
    echo "refined stability localization failed; retained work directory: $work_dir" >&2
  fi
}
trap cleanup EXIT
...
declare -a pids=()
for pair in "400 16" "416 32" "432 48"; do
  read -r total additional <<< "$pair"
  echo "refined stability localization phase: train step $total"
...
for total in 400 416 432; do
  model="$work_dir/model-step-${total}.nsrlpm"
  echo "refined stability localization phase: audit step $total"
