...
const root = process.argv[2] ?? "data/experiments/literary-shared-trunk-moe-v1";
const out = process.argv[3] ?? path.join(root, "shared-trunk-report.json");
...
for (const author of authors) {
  const artifact = await readFile(path.join(root, "experts", author, "bias.nsrlbe"));
...
if (new Set(authors.map((author) => experts[author].artifact_sha256)).size !== authors.length) {
  throw new Error("bias expert artifacts are not unique");
}
const oracleBytes = await readFile(path.join(root, "oracles", "final-report.json"));
const oracle = JSON.parse(oracleBytes);
if (`0x${oracle.trunk.model_hash.slice(2)}` !== trunkModelHash) {
  throw new Error("oracle/training trunk hash mismatch");
}
if (oracle.trunk.forward_count * 3 !== oracle.trunk.naive_three_model_forward_count) {
  throw new Error("shared-trunk forward accounting mismatch");
}

const fixed = oracle.fixed_experts[oracle.best_fixed_expert];
const token = oracle.oracle_routes.token;
const report = {
  schema: "nsrl.literary_shared_trunk_moe.v1",
  trunk: {
  ...
  expert_type: "i32_q8_logit_bias_probability_error_adapter",
  ...
  promotion: {
    ...
    reason: "token oracle ceiling improves error by only 57 Q15 and accuracy by 0 per mille",
    next_expert_type: "fine_grained_hidden_or_low_rank_ffn_residual_adapter",
  },
  source_sha256: { oracle: sha256(oracleBytes) },
  known_non_claims: [
    "bias_experts_are_not_ffn_experts",
