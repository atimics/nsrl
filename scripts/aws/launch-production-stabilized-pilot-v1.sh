...
usage() {
  cat <<'USAGE'
Prepare or launch the production-model-v1 stabilized p10m pilot on EC2 Graviton.

Dry-run is the default and does not call AWS. Execute mode spends money.

Required for --execute:
  NSRL_AMI_ID=ami-...
  NSRL_PRODUCTION_STABILIZED_PILOT_ARTIFACT_S3_URI=s3://bucket/prefix/artifact.tar.gz
  NSRL_PRODUCTION_STABILIZED_PILOT_S3_URI=s3://bucket/prefix/run

Optional:
  NSRL_PRODUCTION_STABILIZED_PILOT_INSTANCE_TYPE=c8g.2xlarge
...
run_name="${NSRL_PRODUCTION_STABILIZED_PILOT_RUN_NAME:-production-p10m-stabilized-pilot-${timestamp}}"
ami_id="${NSRL_AMI_ID:-ami-required-for-execute}"
instance_type="${NSRL_PRODUCTION_STABILIZED_PILOT_INSTANCE_TYPE:-c8g.2xlarge}"
iam_profile="${NSRL_IAM_INSTANCE_PROFILE:-NSRLTrainingEc2InstanceProfile}"
artifact_uri="${NSRL_PRODUCTION_STABILIZED_PILOT_ARTIFACT_S3_URI:-s3://nsrl-training-022118847419-us-east-1/production-model-v1/artifacts/stabilized-pilot-v1.tar.gz}"
run_s3_uri="${NSRL_PRODUCTION_STABILIZED_PILOT_S3_URI:-s3://nsrl-training-022118847419-us-east-1/production-model-v1/runs/${run_name}}"
...
if ((execute == 1)) && [[ -z "${NSRL_PRODUCTION_STABILIZED_PILOT_ARTIFACT_S3_URI:-}" || -z "${NSRL_PRODUCTION_STABILIZED_PILOT_S3_URI:-}" ]]; then
...
#!/bin/bash
set -uo pipefail
exec > >(tee -a /var/log/nsrl-production-stabilized-pilot.log) 2>&1
shutdown -h +240 || true
...
mkdir -p /opt/nsrl /mnt/nsrl/production-model-v1/p10m-stabilized-pilot
aws s3 cp ${artifact_uri} /tmp/nsrl-production-stabilized-pilot.tar.gz
tar -xzf /tmp/nsrl-production-stabilized-pilot.tar.gz -C /opt/nsrl
...
export NSRL_PRODUCTION_STABILIZED_PILOT_S3_URI=${run_s3_uri}
cd /opt/nsrl
set +e
scripts/aws/run-production-stabilized-pilot-v1-graviton.sh
status=\$?
set -e
aws s3 cp /var/log/nsrl-production-stabilized-pilot.log ${run_s3_uri}/launch.log --only-show-errors || true
...
const value = {
  schema: "nsrl.production_stabilized_pilot_aws_launch.v1",
...
if ((execute == 0)); then
  echo "production_stabilized_pilot_launch_plan: $launch_json"
  echo "production_stabilized_pilot_user_data: $user_data"
  exit 0
fi
...
echo "production_stabilized_pilot_instance: $INSTANCE_ID"
echo "production_stabilized_pilot_run_s3_uri: $run_s3_uri"
echo "production_stabilized_pilot_launch_result: $launch_result"
