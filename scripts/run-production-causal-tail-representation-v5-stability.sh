...
name="p10m-causal-tail-representation-v5-stability"
...
cleanup() {
  ...
  if ((status == 0)); then
    ...
    echo "representation-v5 stability run failed; retained work directory: $work_dir" >&2
  fi
}
trap cleanup EXIT
...
echo "representation-v5 stability phase: train 2,048 windows"
...
echo "representation-v5 stability phase: development and health audit"
