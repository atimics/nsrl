...
assert(contract.schema === "nsrl.production_direct_head_nll_safe_set_contract.v1"
  && confirmation.schema
    === "nsrl.production_direct_head_nll_safe_set_confirmation_gate.v1"
...
"direct-head safe-set confirmation gate is invalid");
...
"direct-head safe-set open-generation schema is invalid");
...
const result = {
  schema: "nsrl.production_direct_head_nll_safe_set_quality_gate.v1",
...
if (config.check) {
  assert(fs.readFileSync(config.out, "utf8") === output,
    "direct-head safe-set quality gate does not byte-replay");
} else {
