...
const schema = "nsrl.solomon_aws_prelaunch_readiness_check.v1";
...
const amiIdPattern = /^ami-[0-9a-f]{8,}$/;
const gravitonInstance = /^(?:c|m|r|t)(?:6|7|8)g[dn]?\./;
const subnetPattern = /^subnet-[0-9a-f]{8,}$/;
const securityGroupPattern = /^sg-[0-9a-f]{8,}$/;
const iamProfilePattern = /^[A-Za-z0-9+=,.@_-]{1,128}$/;
...
function usage() {
  console.log([
    "Usage: check-solomon-aws-prelaunch-readiness.mjs --launch-dir PATH [options]",
    "   or: check-solomon-aws-prelaunch-readiness.mjs --launch PATH --user-data PATH",
    "",
    "Checks the no-spend EC2 launch manifest is ready for a real Solomon",
    "Graviton product run: real-looking AMI, Graviton instance family, S3-backed",
    "artifact inputs, IAM profile, IMDSv2, tags, and product CPU scaling.",
    "",
    "Options:",
    "  --out PATH",
    "  --allow-execute-plan",
...
function requireCommandContains(command, text, errors) {
...
function commandHasTag(command, key, value) {
  return command.some((item) => String(item).includes(`Key=${key},Value=${value}`));
}
...
function requirePostRunProofCommand(launch, config, errors) {
  const proofCommand = Array.isArray(launch.post_run_proof_command)
  ...
  const s3Index = proofCommand.indexOf("--s3-pipeline-uri");
  const launchDirIndex = proofCommand.indexOf("--launch-dir");
  if (!proofCommand.includes("scripts/aws/prove-solomon-product-run.sh")) {
    errors.push("post_run_proof_command missing scripts/aws/prove-solomon-product-run.sh");
  }
  if (s3Index < 0 || proofCommand[s3Index + 1] !== String(launch.s3_pipeline_uri || "")) {
    errors.push("post_run_proof_command does not use launch s3_pipeline_uri");
  }
  if (launchDirIndex < 0 || !proofCommand[launchDirIndex + 1]) {
    errors.push("post_run_proof_command missing --launch-dir");
  } else if (path.resolve(proofCommand[launchDirIndex + 1]) !== expectedLaunchDir) {
    errors.push(`post_run_proof_command launch dir ${proofCommand[launchDirIndex + 1]} != ${expectedLaunchDir}`);
  }
  if (!proofCommand.includes("--require-launch-dir")) {
...
function check(config) {
  const errors = [];
  const launch = JSON.parse(fs.readFileSync(config.launchPath, "utf8"));
  ...
  if (config.requireDryRun && launch.dry_run !== true) {
    errors.push("launch dry_run is not true; prelaunch readiness must be no-spend by default");
  }
  if (!amiIdPattern.test(String(launch.ami_id || ""))) {
    errors.push(`ami_id ${JSON.stringify(launch.ami_id || "")} is not a real-looking AMI id`);
  }
  if (String(launch.ami_id || "") === "ami-required-for-execute") {
    errors.push("ami_id is still the execute placeholder");
  }
  if (!gravitonInstance.test(String(launch.instance_type || ""))) {
    errors.push(`instance_type ${JSON.stringify(launch.instance_type || "")} is not a Graviton family`);
  }
  if (!iamProfilePattern.test(String(launch.iam_instance_profile || ""))) {
    errors.push(`iam_instance_profile ${JSON.stringify(launch.iam_instance_profile || "")} is not a valid profile name`);
  }
  if (launch.subnet_id && !subnetPattern.test(String(launch.subnet_id))) {
    errors.push(`subnet_id ${JSON.stringify(launch.subnet_id)} is not a valid subnet id`);
  }
  for (const securityGroup of securityGroups) {
    if (!securityGroupPattern.test(securityGroup)) {
      errors.push(`security_group_id ${JSON.stringify(securityGroup)} is not a valid security group id`);
    }
  }
  ...
  if (
    launch.s3_uri &&
    launch.artifact_s3_uri &&
    !String(launch.artifact_s3_uri).startsWith(`${String(launch.s3_uri).replace(/\/+$/, "")}/`)
  ) {
    errors.push(`artifact_s3_uri ${launch.artifact_s3_uri} is not under ${launch.s3_uri}/`);
  }

  const actualHash = sha256(userData);
  if (String(launch.user_data_sha256 || "") !== actualHash) {
    errors.push(`user_data_sha256 ${JSON.stringify(launch.user_data_sha256 || "")} != ${actualHash}`);
  }
  if (path.resolve(String(launch.user_data || "")) !== path.resolve(config.userDataPath)) {
  ...
  for (const text of [
    ...
    "run-instances",
    "--image-id",
    String(launch.ami_id || ""),
    "--instance-type",
    String(launch.instance_type || ""),
    "--iam-instance-profile",
    `Name=${launch.iam_instance_profile || ""}`,
    "--metadata-options",
    ...
    requireCommandContains(command, text, errors);
  }
  ...
  if (!commandHasTag(command, "Name", launch.run_name || "")) {
    errors.push("launch command missing Name tag for run_name");
  }
  if (!commandHasTag(command, "Project", "nsrl-solomon")) {
    errors.push("launch command missing Project=nsrl-solomon tag");
  }
  if (!commandHasTag(command, "Product", "solomon-v1")) {
    errors.push("launch command missing Product=solomon-v1 tag");
  }
