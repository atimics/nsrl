...
const DEFAULT_OUT = path.join(ROOT, "web/launches/localnet.json");
...
function main() {
  ...
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nsrl-model-localnet-site-"));
  const { snapshot } = buildDeterministicLocalnetDemo(directory);
  const output = `${JSON.stringify(snapshot, null, 2)}\n`;
  if (options.check) {
    if (!fs.existsSync(options.out) || fs.readFileSync(options.out, "utf8") !== output) {
      throw new Error(
        `${path.relative(ROOT, options.out)} is stale; rebuild it with scripts/build-model-localnet-site.mjs`,
      );
    }
