...
out_dir="${NSRL_PRODUCTION_TARGET_PROBABILITY_OUT_DIR:-data/experiments/production-model-v1/p10m-target-probability-resolution}"
checkpoint_out="${NSRL_PRODUCTION_TARGET_PROBABILITY_CHECKPOINT_OUT:-benchmarks/production-model-v1/p10m-target-probability-resolution.json}"
...
if [[ ! -s "$out_dir/audit.json" ]]; then
  "$binary" probability-resolution-audit \
...
node scripts/freeze-production-target-probability-resolution-v1.mjs \
  --run-dir "$out_dir" --out "$checkpoint_out"
echo "production-model-v1 p10m target-probability resolution review completed"
