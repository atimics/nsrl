...
const DEFAULT_OUT = path.join(ROOT, "web/launches/automation.json");
...
function main() {
  ...
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nsrl-bounty-automation-site-"));
  const demo = buildDeterministicBountyAutomationDemo(directory);
  const output = `${JSON.stringify(
    {
      ...demo.snapshot,
      waiting_plan: demo.waiting_plan,
      keeper_result: demo.result,
    },
    null,
    2,
  )}\n`;
  if (options.check) {
    if (!fs.existsSync(options.out) || fs.readFileSync(options.out, "utf8") !== output) {
      throw new Error(
        `${path.relative(ROOT, options.out)} is stale; rebuild it with scripts/build-bounty-automation-site.mjs`,
      );
    }
