...
name="p10m-causal-tail-representation-v4-stability"
...
cleanup() {
  ...
  if ((status == 0)); then
    ...
    echo "representation-v4 stability run failed; retained work directory: $work_dir" >&2
  fi
}
trap cleanup EXIT
...
echo "representation-v4 stability phase: train 2,048 windows"
"$binary" full-train-smoke \
  ...
  --backward-quantization late-stochastic --backward-stochastic-seed 29

echo "representation-v4 stability phase: development and health audit"
