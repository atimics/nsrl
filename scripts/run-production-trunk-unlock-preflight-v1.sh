...
out_dir="${NSRL_PRODUCTION_TRUNK_UNLOCK_OUT_DIR:-data/experiments/production-model-v1/p10m-trunk-unlock-preflight}"
checkpoint_out="${NSRL_PRODUCTION_TRUNK_UNLOCK_CHECKPOINT_OUT:-benchmarks/production-model-v1/p10m-trunk-unlock-preflight.json}"
...
for index in 0 1 2 3; do
  required=("model-$index.nsrlpm" "optimizer-$index.nsrlpo" "train-$index.json" \
    "dev-$index.json" "state-$index.json" "event-$index.json")
  complete=1
  ...
  if ((index == 0)); then
    ...
    model_in="$out_dir/model-$((index - 1)).nsrlpm"
    optimizer_in="$out_dir/optimizer-$((index - 1)).nsrlpo"
    state_in="$out_dir/state-$((index - 1)).json"
  fi
  train_args=(
    full-train-smoke --tokenizer "$tokenizer" --tokens "$train_tokens"
    --model "$model_in" --model-out "$out_dir/model-$index.nsrlpm.tmp"
    --optimizer-state-out "$out_dir/optimizer-$index.nsrlpo.tmp"
    --trace "$out_dir/train-$index.json.tmp"
  ...
  "$binary" "${train_args[@]}"
  mv "$out_dir/model-$index.nsrlpm.tmp" "$out_dir/model-$index.nsrlpm"
  mv "$out_dir/optimizer-$index.nsrlpo.tmp" "$out_dir/optimizer-$index.nsrlpo"
  mv "$out_dir/train-$index.json.tmp" "$out_dir/train-$index.json"
  "$binary" evaluate \
    --tokenizer "$tokenizer" --tokens "$dev_tokens" \
    --model "$out_dir/model-$index.nsrlpm" --trace "$out_dir/dev-$index.json.tmp" \
    --context-tokens 64 --max-windows 256
  mv "$out_dir/dev-$index.json.tmp" "$out_dir/dev-$index.json"
  check_args=(
    --trace "$out_dir/train-$index.json" --interval "$index"
    --state-out "$out_dir/state-$index.json.tmp"
    --event-out "$out_dir/event-$index.json.tmp"
    --dev-initial "$out_dir/dev-initial.json" --dev-current "$out_dir/dev-$index.json"
  ...
  status=$?
  set -e
  mv "$out_dir/state-$index.json.tmp" "$out_dir/state-$index.json"
  mv "$out_dir/event-$index.json.tmp" "$out_dir/event-$index.json"
  if ((status != 0)); then
    echo "trunk-unlock candidate died at interval $index" >&2
    exit "$status"
  fi
done
...
node scripts/freeze-production-trunk-unlock-preflight-v1.mjs \
  --run-dir "$out_dir" --out "$checkpoint_out"
echo "production-model-v1 p10m trunk-unlock preflight completed"
