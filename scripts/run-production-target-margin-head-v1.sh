...
name="p10m-target-margin-head-v1"
...
echo "target-margin phase: frozen 64-window feature-shift preflights"
for feature_shift in 13 14 15; do
  ...
  "$binary" target-margin-train \
    ...
    --spread-windows --max-windows 64 --evaluation-windows 64 \
    --epochs 1 --feature-shift "$feature_shift" --margin-q8 8 \
    --batch-windows 4 --max-optimizer-steps 16
done

node scripts/select-production-target-margin-preflight-v1.mjs \
  --contract "$contract" \
...
echo "target-margin selected feature shift: $selected_shift"

echo "target-margin phase: uninterrupted 2048-window pilot"
"$binary" target-margin-train \
  --tokenizer "$tokenizer" --tokens "$train_tokens" \
  --model "$source_model" \
  ...
  --context-tokens 64 --targets-per-window 8 --training-workers 8 \
  --spread-windows --max-windows 2048 --evaluation-windows 64 \
  --epochs 1 --feature-shift "$selected_shift" --margin-q8 8 \
  --batch-windows 4 --max-optimizer-steps 512

echo "target-margin phase: exact midpoint restart replay"
"$binary" target-margin-train \
  --tokenizer "$tokenizer" --tokens "$train_tokens" \
  --model "$source_model" \
...
echo "target-margin phase: public development stop/go gate"
...
node scripts/check-production-target-margin-development-v1.mjs \
...
echo "target-margin phase: public test and open-generation confirmation"
"$binary" evaluate-canonical \
...
for role in source candidate; do
  ...
  target/release/nsrl-production-rollout-divergence-audit \
    ...
    --trace "$open_generation_dir/${name}-${role}-rollout-divergence.json" \
...
node scripts/freeze-production-target-margin-quality-v1.mjs \
  ...
  --source-rollout "$open_generation_dir/${name}-source-rollout-divergence.json" \
  --candidate-rollout "$open_generation_dir/${name}-candidate-rollout-divergence.json" \
