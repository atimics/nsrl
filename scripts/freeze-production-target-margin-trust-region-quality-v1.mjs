...
assert(contract.schema === "nsrl.production_target_margin_trust_region_contract.v1"
  && confirmation.schema
    === "nsrl.production_target_margin_trust_region_confirmation_gate.v1"
...
"target-margin trust-region confirmation gate is invalid");
...
"target-margin trust-region open-generation schema is invalid");
...
const result = {
  schema: "nsrl.production_target_margin_trust_region_quality_gate.v1",
...
if (config.check) {
  assert(fs.readFileSync(config.out, "utf8") === output,
    "target-margin trust-region quality gate does not byte-replay");
} else {
