...
out_dir="${NSRL_PRODUCTION_KV_PILOT_OUT_DIR:-data/experiments/production-model-v1/p10m-kv-boundary-pilot}"
checkpoint_out="${NSRL_PRODUCTION_KV_PILOT_CHECKPOINT_OUT:-benchmarks/production-model-v1/p10m-kv-boundary-pilot.json}"
...
for interval in 0 1 2 3 4 5 6 7; do
  ...
  if ((complete == 1)); then
    echo "K+V boundary pilot reuse interval $interval"
    continue
  fi
  ...
  train_chunk "$model_in" "$optimizer_in" \
    ...
    "$out_dir/train-$interval.json" 32
  ...
  if ((status != 0)); then
    echo "K+V boundary pilot died at interval $interval" >&2
    exit "$status"
  fi
done
