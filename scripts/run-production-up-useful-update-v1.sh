...
out_dir="${NSRL_PRODUCTION_UP_USEFUL_OUT_DIR:-data/experiments/production-model-v1/p10m-up-useful-update}"
checkpoint_out="${NSRL_PRODUCTION_UP_USEFUL_CHECKPOINT_OUT:-benchmarks/production-model-v1/p10m-up-useful-update.json}"
...
for interval in 0 1 2 3 4 5 6 7; do
  ...
  if ((complete == 1)); then
    echo "up useful-update reuse interval $interval"
    continue
  fi
  ...
  if ((status != 0)); then
    echo "up useful-update candidate died at interval $interval" >&2
    exit "$status"
  fi
done
...
node scripts/freeze-production-up-useful-update-v1.mjs \
  --run-dir "$out_dir" --out "$checkpoint_out"
echo "production-model-v1 p10m up useful-update experiment completed"
