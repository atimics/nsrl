...
name="p10m-causal-tail-context-v1"
...
export NSRL_K_LEARNING_RATE_SHIFT=23
...
target/release/nsrl-production-rollout-divergence-audit \
  ...
  --model "$model" \
  --trace "benchmarks/open-generation-v1/${name}-rollout-divergence.json" \
...
target/release/nsrl-production-context-sensitivity-audit \
  ...
  --trace "benchmarks/open-generation-v1/${name}-context-sensitivity.json" \
  --top-k 8
target/release/nsrl-production-residual-saturation-audit \
  --manifest benchmarks/open-generation-v1/manifest.tsv \
  --tokenizer "$tokenizer" --model "$model" \
  --trace "benchmarks/open-generation-v1/${name}-residual-saturation.json"
