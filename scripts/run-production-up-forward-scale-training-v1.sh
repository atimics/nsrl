...
out_dir="${NSRL_PRODUCTION_UP_FORWARD_TRAIN_OUT_DIR:-data/experiments/production-model-v1/p10m-up-forward-scale-training}"
checkpoint_out="${NSRL_PRODUCTION_UP_FORWARD_TRAIN_CHECKPOINT_OUT:-benchmarks/production-model-v1/p10m-up-forward-scale-training.json}"
...
for interval in 0 1 2 3; do
  ...
  if ((complete == 1)); then
    echo "up forward-scale training reuse interval $interval"
    continue
  fi
  ...
  check_args=(
    ...
    --require-trunk-update-by-interval 1
  ...
  if ((status != 0)); then
    echo "up forward-scale training candidate died at interval $interval" >&2
    exit "$status"
  fi
done
...
echo "up forward-scale training selected interval $selected_interval at $selected_steps steps"
...
node scripts/freeze-production-up-forward-scale-training-v1.mjs \
  --run-dir "$out_dir" --out "$checkpoint_out"
echo "production-model-v1 p10m up forward-scale training completed"
