...
name="p10m-causal-tail-representation-v9-health-scale"
...
trigger_dir="data/experiments/production-model-v1/p10m-causal-tail-representation-v9-health-trigger/trigger"
...
for interval in 0 1 2 3; do
  ...
  if [[ -d "$chunk_dir" ]]; then
    for required in model.nsrlpm optimizer.nsrlpo train.json; do
      if [[ ! -f "$chunk_dir/$required" ]]; then
        echo "health scale chunk $interval is incomplete: $chunk_dir" >&2
        exit 1
      fi
    done
    echo "health scale reuse durable chunk $interval"
    continue
  fi

  if ((interval == 0)); then
    model_in="$trigger_dir/model.nsrlpm"
    optimizer_in="$trigger_dir/optimizer.nsrlpo"
  else
  ...
  echo "health scale phase: chunk $interval (${chunk_steps[$interval]} optimizer steps)"
  if ! "$binary" "${common_args[@]}" \
      --max-optimizer-steps "${chunk_steps[$interval]}" \
      --model "$model_in" --optimizer-state "$optimizer_in" \
      --model-out "$work_dir/model.nsrlpm" \
      --optimizer-state-out "$work_dir/optimizer.nsrlpo" \
      --trace "$work_dir/train.json"; then
    echo "health scale chunk $interval failed; retained work directory: $work_dir" >&2
    exit 1
  fi
...
echo "health scale phase: development and numeric-health audit"
...
node scripts/freeze-production-representation-health-scale-v1.mjs \
