...
name="p10m-causal-tail-representation-v2-stability-localization"
...
source_model="data/experiments/production-model-v1/p10m-causal-tail-representation-v2/midpoint.nsrlpm"
source_optimizer="data/experiments/production-model-v1/p10m-causal-tail-representation-v2/midpoint.nsrlpo"
...
cleanup() {
  ...
  if ((status == 0)); then
    ...
    echo "stability localization failed; retained work directory: $work_dir" >&2
  fi
}
trap cleanup EXIT
...
declare -a pids=()
for pair in "320 64" "384 128" "448 192"; do
  read -r total additional <<< "$pair"
  echo "stability localization phase: train step $total"
...
for total in 320 384 448; do
  model="$work_dir/model-step-${total}.nsrlpm"
  echo "stability localization phase: audit step $total"
