...
- The exact six-atom proposal cube and the preregistered confirmation on
  ...
  M4 then widens the frame to 104 publication-level sources across Federal
  Register, Gutenberg, RFC, and open-access science, with four nonoverlapping
  passages per source and 19 conformal sources per family. Its overall verdict
  is inconclusive: 14/16 evaluation panels are covered, with both failures in
  Gutenberg. Twelve passages fire across three families, all are favorable,
  aggregate contrast is `-63,541` Q32, and no firing is unsafe. Federal
  Register and RFC pass their frozen family gates; Gutenberg is withheld by
  coverage and science abstains. This does not broaden M3 into an arbitrary-
  source certificate. Documents `200--212` remain sealed and do not count as
  sources.
