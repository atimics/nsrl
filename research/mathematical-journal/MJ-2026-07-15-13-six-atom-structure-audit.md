...
A clean checkout reconstructs every integer cube and byte-replays the derived
harmonic, document-Ising, and confirmation results from the hash-bound structure
artifacts. The model and corpus bytes used for the original exact forwards are
not repository artifacts, so a clean checkout verifies the published cubes and
their consequences; it does not rerun the original 8,192 forwards. The frozen
contract still binds the source and executable fingerprints used for that run.
