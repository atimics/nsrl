...
*72 Goetic spirit seals generated by an integer-only text-conditioned bitmap
denoiser (`NSRLTCH`) — i8 weights, u8 ink, i16 features, i64 accumulation, and
zero floating-point operations in training or sampling. The README image uses an
8-column layout of the same 72-target run so each 128×128 seal stays readable;
see [docs/solomon-seal-denoise.md](docs/solomon-seal-denoise.md).*
